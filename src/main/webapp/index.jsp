    ...
    <div class="flex flex-row flex-wrap justify-center p-4 gap-4">
        <% for (Marca marca : marcas) { %>
			<div class="flex flex-col items-center">
				<a href="marca-producto?idMarca=<%=marca.getIdMarca()%>">
					<img class=" w-24 h-24 rounded-full aspect-square"
					src="<%=marca.getImagenUrl()%>" alt="">
				</a>
        ...
        <section class="w-full px-4">
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
                ...
                <div class="bg-white p-4 rounded-lg border border-gray-300 hover:shadow-lg transition flex flex-col items-center">
                    <img class="rounded-md mb-2 object-cover" src="<%=producto.getImagenUrl()%>" alt="<%=producto.getNombre()%>" width="120">
                    <span class="text-sm text-gray-500"><%=presentacion.getNombre()%></span>
                    <p class="text-center font-bold text-gray-700"><%=producto.getNombre()%></p>
                    <span class="text-lg font-semibold text-green-700">S/ <%=String.format("%.2f", producto.getPrecioUnitario()).replace(',', '.')%></span>
                    <button class="w-full h-10 bg-green-700 text-white font-semibold rounded-md hover:bg-green-600 transition mt-2">
                        Agregar al carrito
                    </button>
                </div>
                <% 
